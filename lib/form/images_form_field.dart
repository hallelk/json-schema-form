...
class ImagesFormField extends FormField<List<BaseImage>> {
  ...
  ImagesFormField({
    ...
    this.isEnabled = true,
    List<BaseImage>? value,
    this.icon,
    this.mode = PickerMode.camera,
  }) : super(
        autovalidateMode: AutovalidateMode.disabled,
        initialValue: List<BaseImage>.from(value ?? [], growable: true),
        builder: (FormFieldState<List<BaseImage>?> state) {
          return _ActualField(
            key: key,
            enabled: isEnabled, 
...
class _ActualField extends StatefulWidget {
  final List<BaseImage>? initialValue;
  final bool enabled;
...
class _ActualFieldState extends State<_ActualField> {
  ...
  late List<BaseImage> value;

  @override 
  void initState() {
    super.initState();
    value = widget.initialValue ?? List<BaseImage>.empty(growable: true);
  }
          
  void _doDeletePicture(BaseImage img) async {
  ...
  void handleAddingImage(ImageSource source) async {
    if (value.length == widget.maxAllowed) { 
      ...
      if (pic != null) {
        widget.onChanged(value..add(const BaseImage.empty()));
        ...
        if (mounted) {
          ...
          if (base64pic != null) {
            widget.onChanged(
              value..add(
                BaseImage(location: base64pic, thumbnailLocation: base64pic)
              )
            );
          }
        }
  ...
  void handleAddPictureClicked() async {
    if (widget.mode == null) {
